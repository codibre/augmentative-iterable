...
function stepResolveState(
  ...
  if (context.state !== STOP && context.next < augmentList.length) {
...
const addFilterAsync = getAddAugment(Symbol.iterator, augmentativeIterateAsync, IGNORE);
const addMapAsync = getAddAugment(Symbol.iterator, augmentativeIterateAsync, YIELD);
const addTakeWhileAsync = getAddAugment(Symbol.iterator, augmentativeIterateAsync, STOP);
